...
public class Nun4 : MonoBehaviour
{
    public float speed;
    ...
    private void Move()
    {
        ...
        else if (walkingStop == 3)
        {
            obj = new Vector2(47.6f, -2f);
            transform.position = Vector2.MoveTowards(transform.position, obj, speed * Time.deltaTime);
            if (Vector2.Distance(transform.position, obj) < 0.01f)
                walkingStop = 4;
        }
        else if (walkingStop == 4)
        {
            obj = new Vector2(47.6f, -1.2f);
            transform.position = Vector2.MoveTowards(transform.position, obj, speed * Time.deltaTime);
            if (Vector2.Distance(transform.position, obj) < 0.01f)
                walkingStop = 5;
        }
        else if (walkingStop == 5)
        {
            obj = new Vector2(47.6f, -2f);
            transform.position = Vector2.MoveTowards(transform.position, obj, speed * Time.deltaTime);
            if (Vector2.Distance(transform.position, obj) < 0.01f)
